# -*- coding: utf-8 -*-
"""
Created on Wed Oct  5 10:28:18 2022
...
for sonic in sonic_arr:
    ...
    # for sonic in sonic_arr:
        ...
        # fil = 'coare_Inputs_s1.csv'
        # data = np.genfromtxt(path+fil, delimiter=',')
        import pandas as pd
        data_df = pd.read_csv(path+fil)
        ...
        Cp_hdr += 'Ch\tCe\tL\tzeta\tdT_skinx\tdq_skinx\tdz_skin\tUrf\tTrf\tQrf\t'
        Cp_hdr += 'RHrf\tUrfN\tTrfN\tQrfN\tlw_net\tsw_net\tLe\trhoa\tUN\tU10\tU10N\t'
        Cp_hdr += 'Cdn_10\tChn_10\tCen_10\thrain\tQs\tEvap\tT10\tT10N\tQ10\tQ10N\tRH10\t'
        Cp_hdr += 'P10\trhoa10\tgust\twc_frac\tEdis\tdT_warm\tdz_warm\tdT_warm_to_skin\tdu_warm'
        
        np.savetxt(fnameCp,A_Cp,fmt='%.18e',delimiter='\t',header=Cp_hdr)
        # np.savetxt(fnameA,A,fmt='%.18e',delimiter=',',header=A_hdr)
        ...
        SigH_hdr += 'Ch\tCe\tL\tzeta\tdT_skinx\tdq_skinx\tdz_skin\tUrf\tTrf\tQrf\t'
        SigH_hdr += 'RHrf\tUrfN\tTrfN\tQrfN\tlw_net\tsw_net\tLe\trhoa\tUN\tU10\tU10N\t'
        SigH_hdr += 'Cdn_10\tChn_10\tCen_10\thrain\tQs\tEvap\tT10\tT10N\tQ10\tQ10N\tRH10\t'
        SigH_hdr += 'P10\trhoa10\tgust\twc_frac\tEdis\tdT_warm\tdz_warm\tdT_warm_to_skin\tdu_warm'
        
        np.savetxt(fnameSigH,A_SigH,fmt='%.18e',delimiter='\t',header=SigH_hdr)
        # np.savetxt(fnameA,A,fmt='%.18e',delimiter=',',header=A_hdr)
        print('done with sonic '+sonic+ " wind stress, and sigH")
        ...
        Cp_SigH_hdr += 'Ch\tCe\tL\tzeta\tdT_skinx\tdq_skinx\tdz_skin\tUrf\tTrf\tQrf\t'
        Cp_SigH_hdr += 'RHrf\tUrfN\tTrfN\tQrfN\tlw_net\tsw_net\tLe\trhoa\tUN\tU10\tU10N\t'
        Cp_SigH_hdr += 'Cdn_10\tChn_10\tCen_10\thrain\tQs\tEvap\tT10\tT10N\tQ10\tQ10N\tRH10\t'
        Cp_SigH_hdr += 'P10\trhoa10\tgust\twc_frac\tEdis\tdT_warm\tdz_warm\tdT_warm_to_skin\tdu_warm'
        
        np.savetxt(fnameCp_SigH,A_Cp_SigH,fmt='%.18e',delimiter='\t',header=Cp_SigH_hdr)
        # np.savetxt(fnameA,A,fmt='%.18e',delimiter=',',header=A_hdr)
        print('done with sonic '+sonic+ " wind stress, cp, and sigH")
        # print('done with sonic 1')
        # test on signle value
        # A_output=coare36vnWarm_et(Jd[0], U[0], Zu[0], Tair[0], Zt[0], RH[0], Zq[0], P[0], Tsg[0], SW_dn[0], LW_dn[0], Lat[0], Lon[0], Zi[0], Rainrate[0], Ts_depth[0], Ss[0], cp[0], sigH[0], zrf_u, zrf_t, zrf_q)
