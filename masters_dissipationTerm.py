# -*- coding: utf-8 -*-
"""
Created on Mon Sep 11 11:59:30 2023
...
filepath_PC = r"Z:\combined_analysis\OaklinCopyMNode\code_pipeline\Level1_align-despike-interp/"
...
plt.loglog(k_isr,model_absNoise,color='r',label='ISR accounting for ABS noise')
...
z_avg_df = pd.read_csv(z_filepath+"z_airSide_allFall.csv")
#%%
c_primeU = 0.53 #this is for U
c_primeW = 0.65
...
for root, dirnames, filenames in os.walk(filepath): #this is for looping through files that are in a folder inside another folder
    for filename in natsort.natsorted(filenames):
        ...
        for mNode in mNode_arr:
            ...
            if filename.startswith("mNode_Port"+mNode):
...
eps_df.to_csv(file_save_path+"eps_terms_sonic"+mNode+"_MAD_k_UoverZbar.csv")
...
#%%
eps_1 = pd.read_csv(file_save_path+"eps_terms_sonic1_MAD_k_UoverZbar.csv")
...
eps_2 = pd.read_csv(file_save_path+"eps_terms_sonic2_MAD_k_UoverZbar.csv")
...
eps_3 = pd.read_csv(file_save_path+"eps_terms_sonic3_MAD_k_UoverZbar.csv")
...
eps_4 = pd.read_csv(file_save_path+"eps_terms_sonic4_MAD_k_UoverZbar.csv")
...
eps_combined.to_csv(file_save_path+'eps_combinedAnalysis_MAD_k_UoverZbar.csv')
...
print('done')
#%%
eps_combined= pd.read_csv(file_save_path+'eps_combinedAnalysis_MAD_k_UoverZbar.csv')
eps_combined['eps_sonic1_MAD'] = np.where(eps_combined['MAD_criteria_met_1']!=True,np.nan,eps_combined['eps_sonic1'])
eps_combined['eps_sonic2_MAD'] = np.where(eps_combined['MAD_criteria_met_2']!=True,np.nan,eps_combined['eps_sonic2'])
eps_combined['eps_sonic3_MAD'] = np.where(eps_combined['MAD_criteria_met_3']!=True,np.nan,eps_combined['eps_sonic3'])
eps_combined['eps_sonic4_MAD'] = np.where(eps_combined['MAD_criteria_met_4']!=True,np.nan,eps_combined['eps_sonic4'])
