...
### function start
#######################################################################################
# Function for interpolating the paros to the same frequency as the met heads (1 Hz)
...
for root, dirnames, filenames in os.walk(filepath): #this is for looping through files that are in a folder inside another folder
    for filename in natsort.natsorted(filenames):
        ...
        #         U_horiz = np.sqrt((np.array(df_s1_interp['Ur'])**2)+(np.array(df_s1_interp['Vr'])**2))
        #         U_streamwise = np.sqrt((np.array(df_s1_interp['Ur'])**2)+(np.array(df_s1_interp['Vr'])**2)+(np.array(df_s1_interp['Wr'])**2))
        ...
        #         UpWp_bar_s1 = np.mean(Up_s1*Wp_s1)
        #         VpWp_bar_s1 = np.mean(Vp_s1*Wp_s1)
        #         WpTp_bar_s1 = np.mean(Tp_s1*Wp_s1)
        #         WpEp_bar_s1 = np.mean(TKEp_s1*Wp_s1)
        ...
        #     U_horiz_s1_arr.append(U_horiz)
        #     U_streamwise_s1_arr.append(U_streamwise)
...
# UpWp_bar_df["VpWp_bar_s1"] = np.array(VpWp_bar_s1_arr)
# UpWp_bar_df["VpWp_bar_s2"] = np.array(VpWp_bar_s2_arr)
# UpWp_bar_df["VpWp_bar_s3"] = np.array(VpWp_bar_s3_arr)
# UpWp_bar_df["VpWp_bar_s4"] = np.array(VpWp_bar_s4_arr)
# # UpWp_bar_df.to_csv(path_save_L4+"VpWp_bar_allSonics_allFall.csv")
# UpWp_bar_df.to_csv(path_save_L4+"VpWp_bar_allSonics_CombinedAnalysis.csv")
